...
const lsGetStore = () => {
  try { return JSON.parse(localStorage.getItem(LS_KEY_UNITA) || "{}"); } catch { return {}; }
};
...
function exportInventarioCSV(allItems, filename = `inventario-${new Date().toISOString().slice(0,10)}.csv`) {
...
export default function Inventory() {
    ...
    axios.get("/api/inventario").then((r) => {
      const data = r.data.map((it) => {
        ...
        if ((!it.unita || !Array.isArray(it.unita)) && Array.isArray(fromLS)) {
          return { ...it, unita: ensureUnitaLen(fromLS, Math.max(1, it.quantita_totale || 1)) };
        }
        if (Array.isArray(it.unita)) {
          // allinea e salva anche in LS per resilienza
          const aligned = ensureUnitaLen(it.unita, Math.max(1, it.quantita_totale || 1));
  ...
  const affectedBySubcat = (madre, figlia) =>
    inv.filter((i) => i.categoria_madre === madre && i.categoria_figlia === figlia);
  ...
  async function save() {
    try {
      ...
      if (!isAllFilled(formToSave.unita, size)) {
        alert("Compila tutti i campi 'Assegna ID' (nessuna unità può essere vuota).");
        return;
      }
      ...
      if (formToSave.id) {
        ...
        const { data: created } = await axios.post("/api/inventario", formToSave);
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <button className="btn btn-primary" onClick={() => setCatsOverlayOpen(true)}>
            Gestisci categorie
          </button>
          <button
          ...
          <tbody>
            {filtered.map((r) => (
              <tr
                ...
                onClick={() => {
                  const unita = ensureUnitaLen(r.unita ?? lsGetUnita(r.id), Math.max(1, r.quantita_totale || 1));
                  setForm({ ...r, unita });
                }}
              >
                <td>{r.nome}</td>
                  ...
                  <div className="inline-flex items-center gap-2">
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        setOverlayItem({
                          ...r,
                          unita: ensureUnitaLen(r.unita ?? lsGetUnita(r.id), Math.max(1, r.quantita_totale || 1)),
                        });
                      }}
                    >
                      ID Assegnati
      ...
      {catsOverlayOpen && (
        <Modal
          ...
          footer={
            <div className="flex items-center gap-2">
              <button className="btn" onClick={() => setCatsOverlayOpen(false)}>Chiudi</button>
              <button
                className="btn btn-primary"
                onClick={() => {
                  setNewCat({ madre: activeMadre, figlia: "" });
                  setCatOpen(true);
                }}
              >
        ...
        >
          <div className="w-[min(98vw,1100px)] max-w-full mx-auto max-h-[70vh] overflow-y-auto">
  ...
  { key: "Interior Design", label: "Interior Design" },
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span>{m.label}</span>
                        <span className="text-xs bg-neutral-200 rounded-full px-2 py-0.5">{count}</span>
                      </div>
                    </button>
                  );
                })}
              ...
              <div className="flex-1">
                ...
                {(() => {
                  ...
                  const figlie = catSearch
                    ? all.filter((n) => n.toLowerCase().includes(catSearch.toLowerCase()))
                    : all;
                  if (figlie.length === 0) {
                    return (
                  ...
                  return (
                    <ul className="space-y-3">
                      {figlie.map((f) => (
                        ...
                        >
                          {catEdit && catEdit.type === 'figlia' && catEdit.madre === activeMadre && catEdit.figlia === f ? (
                            <>
                              <input
                                className="input flex-1"
                                value={catEdit.new_figlia}
                                onChange={(e) => setCatEdit((c) => ({ ...c, new_figlia: e.target.value }))}
                              />
                              <div className="flex items-center gap-2">
                                <button
                                  ...
                                  onClick={async () => {
                                    try {
                                      if (!catEdit.new_figlia || !catEdit.new_figlia.trim()) {
                                ...
                                <button className="btn text-sm px-3 py-1.5" onClick={() => setCatEdit(null)}>
                                  Annulla
                                </button>
                              </div>
                            </>
                            ...
                            <>
                              ...
                              <div className="flex items-center gap-2 shrink-0">
                                <button
                                  ...
                                  onClick={() => setCatEdit({ type: 'figlia', madre: activeMadre, figlia: f, new_figlia: f })}
                                >
                                  Rinomina
                                </button>
                                <button
                                  className="btn-danger text-xs px-3 py-1.5 rounded-full"
                                  onClick={() => setCatDelete({ madre: activeMadre, figlia: f })}
                                >
                                  Elimina
                                </button>
                              </div>
      ...
      {catDelete && (
        <Modal
          ...
          footer={
            <div className="flex items-center gap-2">
              <button className="btn" onClick={() => setCatDelete(null)}>Annulla</button>
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    const attempts = [
                      () => axios.delete("/api/categorie", { data: { madre, figlia } }),
                      () => axios.delete("/api/categorie", { params: { madre, figlia } }),
                      () => axios.delete(`/api/categorie/${encodeURIComponent(madre)}/${encodeURIComponent(figlia)}`),
                      () => axios.post("/api/categorie/delete", { madre, figlia }),
                      () => axios.post("/api/categorie/remove", { madre, figlia }),
                    ];
                    let lastErr = null;
                    for (const run of attempts) {
                      try { await run(); lastErr = null; break; } catch (e) { lastErr = e; }
                    }
  ...
  <option value="Interior Design">Interior Design</option>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <span
                  className="inline-flex items-center justify-center h-9 px-4 min-w-[4rem] text-base font-medium border border-neutral-300 rounded-md bg-white text-center"
                >
      ...
      {idOverlayOpen && form && (
        <Modal
          ...
          footer={
            <button className="btn btn-primary" onClick={() => setIdOverlayOpen(false)}>
              Chiudi
            </button>
          }
        >
          <div className="grid grid-cols-1 gap-3 md:gap-4">
            {Array.from({ length: Math.max(1, form.quantita_totale || 1) }, (_, i) => (
              <input
                key={i}
                className="input"
                placeholder={`Unità ${i + 1}`}
                value={ensureUnitaLen(form.unita, Math.max(1, form.quantita_totale || 1))[i] || ""}
                onChange={(e) =>
                  setForm((f) => {
              ...
              onClick={async () => {
                try {
                  ...
                  if (!isAllFilled(overlayItem.unita, size)) {
                    alert("Compila tutti i nomi/ID delle unità prima di salvare.");
                    return;
                  }
                  const payload = { ...overlayItem, unita: trimToLen(overlayItem.unita, size) };
              ...
              <div key={i} className="flex items-center gap-3 py-1">
                {editingIndex === i ? (
                  <>
                    <input
                      className="input flex-1"
                      value={ensureUnitaLen(overlayItem.unita, Math.max(1, overlayItem.quantita_totale || 1))[i] || ""}
                      onChange={(e) =>
                        setOverlayItem((o) => {
                          const unita = ensureUnitaLen(o.unita, Math.max(1, o.quantita_totale || 1));
                          unita[i] = e.target.value;
                          return { ...o, unita };
                        })
                      }
                  ...
                  <>
                    <span className="flex-1">{ensureUnitaLen(overlayItem.unita, Math.max(1, overlayItem.quantita_totale || 1))[i] || `Unità ${i+1}`}</span>
                    <button
