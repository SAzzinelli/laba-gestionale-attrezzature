...
const Inventory = () => {
 ...
 <div className="flex-1">
 <h3 className="text-lg font-medium text-gray-900 mb-1">{item.nome}</h3>
 {item.note && (
 <p className="text-sm text-gray-500 mb-2">{item.note}</p>
 )}
 <div className="space-y-1 text-sm text-gray-600">
 <div><strong>Categoria:</strong> {item.categoria_madre} - {item.categoria_figlia}</div>
 <div><strong>Corso:</strong> {item.corsi_assegnati ? (Array.isArray(item.corsi_assegnati) ? item.corsi_assegnati.join(', ') : item.corsi_assegnati) : '-'}</div>
 </div>
 </div>
 ...
 <div><strong>Quantità:</strong> {item.quantita_totale}</div>
 <div><strong>Scaffale:</strong> {item.scaffale || 'N/A'}</div>
 </div>
 </div>
 ))}
 </div>
